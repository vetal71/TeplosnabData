...
CREATE     PROCEDURE [dbo].[sp_calc_kot]
...
AS	
	...
	BEGIN		
		...
		SET
			...
			gkt = ROUND(c.q*a.kdo*a.kco*(c.t-a.srt)/(c.t+26)/1000000,2),
			st = 1
		FROM
			datakoteln as a inner join dataobekt as b on (a.datan=b.datan) join
    	obekt as c on (a.kodkot=c.kodkot) and (c.kodobk=b.kodobk)
    	WHERE (a.kodkot=@kod and b.datan=@data and c.podkl=0) and (b.st Is Null or b.st = 1)
		-- Обновляем дома
		UPDATE datadoma
		SET
			gkt = ROUND(c.so*a.normativ_ot,2), st = 1
		FROM
			datakoteln a inner join datadoma as b on (a.datan=b.datan) join
	...
	BEGIN
		...
		FROM
			datakoteln as a inner join dataobekt as b on (a.datan=b.datan) join
    	obekt as c on (a.kodkot=c.kodkot) and (c.kodobk=b.kodobk)
    	WHERE (a.kodkot=@kod and b.datan=@data and c.prj!=0) and (b.sv Is Null or b.sv = 1)
		-- Обновляем дома
		UPDATE datadoma
		SET
			gkv = ROUND(c.prj*a.n_gv*a.normativ_gv,2), sv = 1
    	...
    	doma as c on (a.kodkot=c.kodkot) and (c.koddom=b.koddom)
    	WHERE (a.kodkot=@kod and b.datan=@data and c.podklgv=0) and (b.sv Is Null or b.sv = 1)
	END
